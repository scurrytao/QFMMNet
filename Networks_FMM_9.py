...
# from GFMM import FuzzyMinMaxNN
import numpy as np
...
class HGNN(nn.Module):
    ...
    def Quality(self, b, u):
        ...
        for key, value in b.items():
            # 计算每个（125,2）tensor的最大值，并转换为（125,1）tensor
            max_value = torch.max(value, dim=1, keepdim=True)[0]
            # 将最大值添加到新的字典中
            b_max[key] = max_value
        for i in range(len(u)):
            Q[i] = -(torch.log(u[i]) * self.views * torch.exp(b_max[i]) / (u_all))
            # Q[i] = -(torch.log(u[i]) * 3 * torch.exp(b_max[i]) / (u_all * u[i]))
            # Q[i] = -(torch.log(u[i]) * torch.exp(b_max[i]) / (u_all))
        return Q
...
def ce_loss(p, alpha, c, global_step, annealing_step):
    S = torch.sum(alpha, dim=1, keepdim=True)    # 返回输入tensor的指定维度dim上的和
    E = alpha - 1
...
def FMM(t, l):
    ...
    for i in range(len(detached_tensor)):
        ...
        new_list.append(a)
        pre.append(b)
    # 再将 NumPy 数组转换为位于 GPU 上的 Tensor
